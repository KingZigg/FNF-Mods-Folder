function onCreate()
  ...
  makeLuaSprite('bg','city/sky',-100,0)
  setScrollFactor('bg',0.1,0.1)
  addLuaSprite('bg',false)
end

  makeLuaSprite('city','city/city',-10,0)
  setScrollFactor('city',0.3,0.3)
  scaleObject('city',0.85,0.85)
  addLuaSprite('city',false)

  makeLuaSprite('window', 'city/window', -10, 0);
  ...
  if not lowQuality then
  makeLuaSprite('streetBehind','city/behindTrain',-40,50)
  addLuaSprite('streetBehind',false)
end
  makeLuaSprite('train','city/train',2000,360)
  addLuaSprite('train',false)

  precacheSound('train_passes')

  makeLuaSprite('street','city/street',-40,50)
...
function onBeatHit()
	...
	if curBeat % 4 == 0 then
		...
		curLight = getRandomInt(0, 3);
		setProperty('window.visible', true)
		setProperty('window.alpha', 1)
		setProperty('window.color', getColorFromHex(cityLightsColors[getRandomInt(0, #cityLightsColors)]));
	end
